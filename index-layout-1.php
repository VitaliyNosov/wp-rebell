<?php
    ...
    $blog_with_sidebar = "";
    if ( (isset($rebell_theme_options['sidebar_blog_listing'])) && ($rebell_theme_options['sidebar_blog_listing'] == "1" ) ) $blog_with_sidebar = "yes";
...
<div id="primary" class="content-area">                    
    ...
    <div id="content" class="site-content blog" role="main">             
		...
		<?php endif; ?>
			...
			<div class="row">
				<div class="xlarge-8 large-10 xlarge-centered large-centered columns without-sidebar index-layout-1">
					...
					<?php 
						if ( is_category() ) : ?>
							<div class="page-type page-title-desc"><?php esc_html_e( 'Category Archives', 'rebell' ); ?></div>
	            		...
	            		<li class="cat-item <?php echo esc_attr( $current_cat ); ?>">
							<a href="<?php if ( get_option( 'show_on_front' ) == 'page' ) echo get_permalink( get_option('page_for_posts' ) );
								else echo esc_url( home_url() );?>"><?php esc_html_e( 'All', 'rebell'); ?>
							</a>
						</li>
		...
		<div class="row">
			...
			<?php if ( (isset($rebell_theme_options['sidebar_blog_listing'])) && ($rebell_theme_options['sidebar_blog_listing'] == "1" ) ) : ?>
								...
								<li class="blog-post <?php echo get_post_format(); ?>">
									<div class="blog-post-inner">
										...
										<h2 class="entry-title-archive">
											<a href="<?php echo has_post_format('link') ? esc_url( rebell_get_link_url() ) : the_permalink() ; ?>" class="thumbnail_archive">
										...
										<div class="post_meta_archive"><?php rebell_entry_archives(); ?></div>
												
										<div class="entry-content-archive">
											
											<?php if (get_option('rss_use_excerpt') == 0) : ?>
												<?php echo '<p>' . wp_trim_words(get_the_excerpt(), 15) . ' </p>'?>
												<a class="more-link" href="<?php echo get_permalink(); ?>"><?php esc_html_e('Continue Reading', 'rebell'); ?></a> 
											<?php elseif (get_option('rss_use_excerpt') == 1) : ?>
												<?php the_excerpt(); ?>
												<a href="<?php the_permalink(); ?>" class="more-link">
													<?php esc_html_e('Continue Reading', 'rebell'); ?>
												</a>
											<?php else : ?>
												<?php echo '<p>' . wp_trim_words(get_the_excerpt(), 15) . ' </p>'?>
												<a class="more-link" href="<?php echo get_permalink(); ?>"><?php esc_html_e('Continue Reading', 'rebell'); ?></a> 
