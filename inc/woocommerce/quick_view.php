...
if ( GETBOWTIED_WOOCOMMERCE_IS_ACTIVE ) {	
	...
	function getbowtied_product_quick_view_button() {
		...
		if ( (isset($rebell_theme_options['quick_view'])) && ($rebell_theme_options['quick_view'] == 1) ) :
			echo '<a href="#" id="product_id_' . $product->get_id() . '" class="button getbowtied_product_quick_view_button" data-product_id="' . $product->get_id() . '">' . esc_html__( 'Quick View', 'rebell') . '</a>';
		endif;
	}
