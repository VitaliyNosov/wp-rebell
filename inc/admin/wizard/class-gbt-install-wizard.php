...
if ( ! class_exists( 'GBT_InstallWizard' ) ) {
	...
	class GBT_InstallWizard {
		...
		public function setupWizard() {
			...
			$default_steps = array(
				'introduction' => array(
					'name'    => esc_html__( 'Introduction', 'rebell' ),
		...
		public function setup_wizard_steps() {
			...
			<div class="gbt-wizard-logo <?php echo GBTHELPERS::theme_slug(); ?>">
				<a href="#">
					<img src="<?php echo get_template_directory_uri() .'/inc/admin/wizard/images/rebell-logo-w.png'; ?>" <?php echo file_exists(get_template_directory() .'/inc/admin/wizard/images/rebell-logo-w@2x.png')? 'srcset="'.get_template_directory_uri() .'/inc/admin/wizard/images/rebell-logo-w@2x.png 2x'.'"': '' ; ?> alt="Logo">
				</a>
			</div>
		...
		public function gbt_setup_introduction() {
			?>
				<div class="wrapper wizard-introduction" style="background-image:url(<?php echo get_template_directory_uri() .'/inc/admin/wizard/images/rebell-setup.jpg'; ?>);">
					<div class="center">
						<h1><?php esc_html_e( GBTHELPERS::theme_name() . '\'s', 'rebell');?> <br/> <?php esc_html_e('Theme Setup Wizard', 'rebell' ); ?></h1>
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" 
							class="button-primary button button-large button-next">
							<?php esc_html_e( 'Get Started', 'woocommerce' ); ?>
						</a>
						<p><?php esc_html_e( 'The quickest way to setup the theme', 'rebell'); ?> <br /> <?php esc_html_e('and start working on your site.', 'rebell' );?></p>
					</div>
				</div>
			...
			?>
				<div class="wrapper wizard-plugins">
					...
					<div class="content-info">
						<h1><?php esc_html_e( 'Plugin Installation', 'rebell' ); ?></h1>
						<p><?php esc_html_e( 'Install the required plugins before importing the demo content.', 'rebell' ); ?></p>
					</div>
					...
					<ul class="plugins">
						...
						<li class="recommended">
							<h3><?php esc_html_e( 'Recommended', 'rebell' ); ?></h3>
						</li>
						<?php $this->_get_plugins_list($plugins, false); // recommended plugins ?>
					</ul>

					<div class="buttons">
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button skip"><?php esc_html_e( 'Skip', 'rebell' ); ?></a>
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button install "><?php esc_html_e( 'Install', 'rebell' ); ?></a>
					</div>
				</div>
					...
					<li class="plugin <?php echo esc_attr( $slug ); ?>">
						...
						<label for="<?php echo esc_attr($plugin['slug']); ?>" class="<?php echo ( isset($plugin['demo_required']) && $plugin['demo_required'] === true ) ? 'required' : 'optional' ?>">
							...
							<div class="plugin-description-container">
								...
								<p><?php echo isset( $plugin['description'] )? esc_html_e( $plugin['description'], 'rebell' ) : ''; ?></p>
							</div>
							...
							<div class="plugin-install">
								...
								<?php if ( ! empty( $plugin['status'] ) && ($plugin['status'] != 'no-action') ) : ?>
										<a  class="button ajax-request <?php echo esc_html( $plugin['status'] ); ?>-now" 
											...
											<?php echo ( isset($plugin['status']) && $plugin['status'] == 'install' ) ? esc_html_e( 'Install Now', 'rebell' ) : ''; ?>
											<?php echo ( isset($plugin['status']) && $plugin['status'] == 'update' ) ? esc_html_e( 'Update Now', 'rebell' ) : ''; ?>
											<?php echo ( isset($plugin['status']) && $plugin['status'] == 'activate' ) ? esc_html_e( 'Activate', 'rebell' ) : ''; ?>
										</a>

									<?php else : ?>
										<a class="button button-disabled">
											<?php  esc_html_e( 'Active', 'rebell' ); ?>
										</a>
									<?php endif; ?>
								</div>
							</div>
		...
		public function gbt_setup_demo() {
			...
			<div class="wrapper wizard-demo-import">
				...
				<div class="content-info">
					<h1><?php esc_html_e( 'Demo Content Import', 'rebell' ); ?></h1>
					<p><?php esc_html_e( 'Start with pre-built page layouts, dummy product pages,', 'rebell');?><br/> <?php esc_html_e('blog posts and widgets.', 'rebell' ); ?></p>
				</div>
				...
				<div class="demo-icon <?php echo ( ! $is_required_plugins === true) ? 'error' : '' ?>">
					...
					<?php else : ?>
						<p class="error-info"><?php esc_html_e( 'Please make sure Visual Composer and WooCommerce are installed and activated before importing the demo content.', 'rebell' ); ?></p>
					<?php endif; ?>
				</div>
				...
				<div class="buttons">
					...
					<?php if ( $is_required_plugins === true ) : ?>
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button install"><?php esc_html_e( 'Install', 'rebell' ); ?></a>
					<?php else : ?>
						<a href="<?php echo esc_url( $this->get_prev_step_link() ); ?>" class="button "><?php esc_html_e( 'Go back', 'rebell' ); ?></a>
					<?php endif; ?>
				</div>

				<!-- <p class="ocdi__ajax-loader  js-ocdi-ajax-loader">
					<span class="spinner"></span> <?php esc_html_e( 'Importing, please wait!', 'rebell' ); ?>
				</p> -->
		...
		public function gbt_setup_final() {
			...
			<div class="wrapper wizard-ready">
				<div class="content-done" style="background-image:url(<?php echo get_template_directory_uri() .'/inc/admin/wizard/images/rebell-setup.jpg'; ?>);">
					<div class="center">
						<h1><?php esc_html_e( 'Setup has been completed successfully!', 'rebell' ); ?></h1>
						<a href="<?php echo esc_url( site_url() ); ?>" class="button button-primary"><?php esc_html_e( 'View Site', 'rebell' );?></a>
						<p><?php esc_html_e('You should be able to start working on your site now.','rebell');?> <br/>
						<?php esc_html_e('Best of luck with your project!', 'rebell'); ?></p>
					</div>
				</div>
				
				<div class="further-info">
					<h2><?php esc_html_e( 'What\'s next?', 'rebell' );?></h2>
					<div class="column">
						...
						<ul>
							<li>
								<a href="<?php echo esc_url( admin_url( 'customize.php' ) ); ?>">
									...
									<?php esc_html_e( 'Customize the theme', 'rebell' ); ?>
								</a>
							</li>
							<li>
								<a href="<?php echo esc_url( admin_url( 'index.php?page=wc-setup' ) );?>">
									<span class="dashicons dashicons-admin-settings"></span>
									<?php esc_html_e( 'WooCommerce Setup Wizard', 'rebell' ); ?>
								</a>
							</li>
						</ul>
					</div>
						...
						<ul>
							<li>
								...
								<a href="<?php print esc_url( $activate ); ?>">
									<span class="dashicons dashicons-update"></span><?php esc_html_e('Activate Theme Updates', 'rebell' ); ?>
								</a>
							</li>
							<li><a href="<?php echo esc_url( GBTHELPERS::support_link() ); ?>" target="_blank"><span class="dashicons dashicons-editor-help"></span><?php esc_html_e( 'Help Center / Support', 'rebell' ); ?></a></li>
						</ul>
					</div>
				</div>
		...
		public function _get_plugins() {
			...
			$newplugins['rebell-extender'] = $plugins['rebell-extender'];
			$newplugins['rebell-portfolio'] = $plugins['rebell-portfolio'];
