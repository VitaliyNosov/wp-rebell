...
if( GETBOWTIED_WOOCOMMERCE_IS_ACTIVE ) {
	...
	class GBT_Ajax_Search {
		...
		function __construct() {
			...
			if( isset($rebell_theme_options['search_in_titles']) && $rebell_theme_options['search_in_titles'] ) {
						...
						<?php $product = wc_get_product( $product_id ); ?>
							<?php if($product) { ?>
								...
								<li class="column">
									...
									<?php if ( (isset($rebell_theme_options['second_image_product_listing'])) && 
										($rebell_theme_options['second_image_product_listing'] == "1" ) ) {
										...
										<div class="product_thumbnail_wrapper <?php echo esc_attr( $instock_class ); ?>">
											...
											<?php if ( (isset($rebell_theme_options['catalog_mode'])) && ($rebell_theme_options['catalog_mode'] == 0) ) { ?>
												...
												<?php if ( !$product->is_in_stock() && !empty($rebell_theme_options['out_of_stock_label']) ) { ?>       
										            <div class="out_of_stock_badge_loop"><?php echo esc_html( $rebell_theme_options['out_of_stock_label'], 'woocommerce' ); ?></div>        
										        <?php } ?>
										    <?php } ?>

										</div>
								
										<h3><a class="product-title-link" href="<?php echo get_permalink( $product->get_id() ); ?>"><?php echo esc_html($product->get_title()); ?></a></h3> 
